...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        pos_actions = self.getLegalActions(state)
        if len(pos_actions) == 0:
            ...
            for i in range(0, len(pos_actions)):
                action_value = self.getQValue(state, pos_actions[i])
                if action_value > max_action_value:
                    max_action_value = action_value
                    action = pos_actions[i]
            return action
        util.raiseNotDefined()
    ...
    def getAction(self, state):
        ...
        if len(legalActions) != 0:
            best_act = self.getPolicy(state)
            prob = self.epsilon
            toss = util.flipCoin(prob)
            action = best_act
            if toss == True:
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for i in range(0, len(b)):
            summ = summ+self.weights[b[i]]*vals[i]
        return summ
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        for i in range(0, len(b)):
            self.weights[b[i]] = self.weights[b[i]]+self.alpha*diff*vals[i]
        return self.weights
        util.raiseNotDefined()
